...
- 凭据、token、refresh token、Client Secret、密码和未脱敏 API 响应不得写入代码、日志、测试快照、验证摘要或子智能体返回。
...
本项目默认由一个可见主智能体持有目标、授权、当前事实和最终综合责任；独立、边界明确且收益充分的工作才交给临时子智能体并原生返回。本节只登记主任务、项目自定义 agent 配置、唯一工作目录、worktree 策略和紧急规则推送范围；详细流程按 `$codex-workflow-guard` 执行。
<!-- CODEX-MANAGED:thread-reference:END -->

- 协作模式：单主智能体 + 按需子智能体。
- 唯一主项目目录：`C:\Users\dztf6\Documents\美客多折扣管家`。
- 工作树策略：同一 checkout 只允许一个写入者；并行写入使用隔离 worktree。
- 项目主任务标题：美客多折扣管家｜项目主智能体
- 项目主任务真实 ID：`019fe7fe-3673-7862-ba2e-6411596f03b2`
- 项目主任务职责边界：持有本项目最终可用软件目标、授权、当前事实、候选状态和最终综合责任；按需生成临时子智能体。
- 紧急规则推送范围：仅限立即影响高风险动作、安全边界、授权范围或外部写入门槛的规则变化。

只登记当前真实存在且已经确认使用的项目主任务；临时子智能体不登记 ID、状态或回执。

主任务不存在、已归档、不可用或地址未经核实时，标记为“未绑定”，不得猜测或保留过期 ID。旧主管、开发、测试或规则任务不得继续作为项目授权入口。

不得记录任务或子智能体的 idle、active、waiting、notLoaded 等动态状态，也不得在本节复制通用的生成、等待、替代、恢复、失败回流或正式验收流程。
...
- 每次代码改动、功能调整、逻辑修复或热更新同步，必须严格递增版本号（同步更新 package.json、main_window.py、spec 打包配置、测试用例以及安装包 build-info.json/Info.plist），确保客户端界面与运行产物版本清晰明确且完全一致。
